...
import initGuide from '/src/assets/initGuide.png'
  ...
  return (
    ...
    className={`${active ? 'absolute' : 'hidden'} w-full h-full backdrop-blur-sm bg-black bg-opacity-30 z-10 overflow-hidden`}>
        <div className='absolute right-0 top-[20%] w-fit flex gap-3 flex-col items-end'>
          <p className='bg-white text-black max-w-[60%] p-3 rounded-2xl rounded-br-none mr-10'>{guideText[textIndex]}</p>
          <img src={initGuide} alt="하얀색 귀여운 캐릭터" className='w-[100px]'/>
        </div>
    </button>
  )
}